module floatMult (floatA,floatB,product); // 浮点数乘法器
// 注意浮点数是符合IEEE754标准

input [31:0] floatA, floatB;  // 输入两个32位位宽乘法器
output reg [31:0] product;  // 输出一个32位位宽结果

reg sign; // 标志位
reg [7:0] exponent; // 阶码
reg [22:0] mantissa; // 尾数
...
always @ (floatA or floatB) begin
	if (floatA == 0 || floatB == 0) begin
		...
		product = {sign,exponent,mantissa}; // 大括号表示拼接，代表最后输出位
	end
end
